"""文档理解模块：将Markdown/PDF解析结果结构化为章节、子章节与可检索大纲项。"""
...
class Document:
    ...
    async def from_markdown_async(
        ...
        # 12-16) 按输入顺序回收结果，避免并发打乱章节顺序
        for task1, task2 in tasks:
            # 保持输入顺序回填，避免并发导致章节顺序错乱
    ...
    def retrieve(
        ...
        # 按 section -> subsection 两级索引检索大纲所需内容
        subsecs = []
